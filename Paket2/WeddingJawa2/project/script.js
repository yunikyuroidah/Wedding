// Wedding date - Set 30 hari dari sekarang agar countdown berjalan
const today = new Date();
const futureDate = new Date(today.getTime() + (30 * 24 * 60 * 60 * 1000)); // 30 hari dari sekarang
const weddingDate = futureDate.getTime();
...
function updateCountdown() {
    ...
    if (timeLeft > 0) {
        ...
        // Add celebration message above countdown if not exists
        const countdownContainer = document.querySelector('.countdown-container');
        if (countdownContainer && !document.querySelector('.countdown-finished')) {
            const celebrationDiv = document.createElement('div');
            celebrationDiv.className = 'countdown-finished';
            celebrationDiv.innerHTML = `
                <h2>🎉 Hari Bahagia Telah Tiba! 🎉</h2>
                <p>Selamat menikah untuk pasangan yang berbahagia!</p>
            `;
            countdownContainer.parentNode.insertBefore(celebrationDiv, countdownContainer);
        }
    }
}
